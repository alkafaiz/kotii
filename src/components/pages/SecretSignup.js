...
export default function SecretSignup(props) {
  const classes = useStyle();
  const [loading, setLoading] = useState(false);

  const handleSignup = res => {
    const { success, authUser, coupleInfo, id } = res;
    ...
    if (success) {
      ...
      props.history.push(`/login?exist=true`);
    }
  };
    ...
    <Box className={classes.wrapper}>
      <Paper className={classes.paper} variant="outlined" elevation={0}>
        <Typography variant="h5" gutterBottom>
          Welcome! Click here to signup.
        </Typography>
        <Box>
          <Button
            className={classes.googleBtn}
            startIcon={<GoogleIcon />}
            endIcon={loading ? <CheckCircleRoundedIcon /> : null}
            disabled={loading}
            classes={{ label: classes.labelbtn }}
            onClick={() => {
              setLoading(true);
              signupInitialUser(handleSignup);
            }}
          >
            {loading ? (
